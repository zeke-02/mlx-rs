...
use model::{Bert, BertInput, ModelArgs};
...
fn load_model(model_dir: &PathBuf) -> Result<Bert> {
    ...
    // Create the model
    let mut model = Bert::new(&model_args)?;
...
fn extract_embeddings(model: &mut Bert, input_ids: &Array) -> Result<Array> {
    // Forward pass through the model
    let bert_input = BertInput::from(input_ids);
    let hidden_states = model.forward(bert_input)?;
    ...
    let cls_embedding = hidden_states.index((0, 0, ..));
...
fn main() -> Result<()> {
    ...
    let model_dir =
        PathBuf::from("/Users/zekieldee/Desktop/code/embedding-models/all-MiniLM-L6-v2-bf16"); // Change this to your model directory path
    let input_text = "Hello, this is a test sentence for BERT embeddings."; // Change this to your test text
    ...
    for i in 0..10.min(embedding_shape[0]) {
        let val = normalized_embedding.index(i).item::<f32>();
        println!("  dim[{}]: {:.6}", i, val);
    }

    println!("\n[SUCCESS] BERT model loaded and inference completed successfully!");
